data {
  ...
  vector[N_id] bv_mean;              // Breeding values
...
transformed data {
  ...
  vector[N] bv_std_full;
  for (i in 1:N) {
    bv_std_full[i] = (bv_mean[id_idx[i]] - bv_mean_std) / bv_sd_std;
  }
  vector[N] bv2_std_full = square(bv_std_full);
  ...
  matrix[N, 6] x_mat = append_col(append_col(append_col(append_col(append_col(rep_vector(1, N), age_q1_std), age_q2_std), f_std), bv_std_full), bv2_std_full);
}

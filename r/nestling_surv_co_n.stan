...
transformed data {
  ...
  matrix[N, 3] fixed_pred_mat = append_col(append_col(rep_vector(1, N), f_std), co_n_std);
}
...
transformed parameters {
  ...
  vector[5] beta_vec = [alpha_std, beta_f_std, beta_co_n_std, beta_bv_std, beta_bv2_std]';
  // Random intercepts
  vector[N] rand_inter;
  // Predictor matrix
  matrix[N, 5] x_mat;

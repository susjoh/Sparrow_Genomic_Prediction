...
parameters {
  ...
  vector[N_dam] z_dam;              // Std.normal noise for parental random effect
  vector[N_sire] z_sire;              // Std.normal noise for parental random effect
...
transformed parameters {
  ...
  for (i in 1:N) {
    rand_inter[i] = hy[hy_idx[i]] + hi[hi_idx[i]] + dam[dam_idx[i]] + sire[sire_idx[i]] + clutch[clutch_idx[i]];
  }
}
